...
from variables import (VariableInfo, Variables, Origin, _to_torch_type, TorchType)
...
def optimize_script(script: ScriptModule, invocations: Invocations, info: ModuleOptimizationInfo) -> Optional[ScriptModule]:
    ...
    def add_input(name: str, graph_input: Value, var_info: VariableInfo, tp: TorchType):
    ...
    print("OPTIMIZED SCRIPT GRAPH VARIABLES")
    vars.dump_vars()

    # Test that the constants propagated are compatible with the values generated by actually running it
    for i,invocation in enumerate(invocations.calls):
        print("invocation", i, "of", len(invocations.calls))
        scope = Scope(default_find_operation)
        output = scope.exec_graph(graph, invocations.m, invocation.args, invocation.kwargs)

        def check_compatible(name: str, expected: VariableInfo, observed: Any):
            #if name != "self":
            #    print("checking", name, expected, "from", expected.produced_by)
            if expected.produced_by is not None:
                for input in expected.produced_by.inputs():
                    n = input.debugName()
                    val = vars[n]
                    #print("  input", val)
                try:
                    for output in expected.produced_by.outputs():
                        n = output.debugName()
                        val = vars.vars[n]
                        #print("  output", val)
                except:
        ...
        for name,expected in vars.items():
            if name not in scope.var_names:
                if expected.is_optional():
                    continue # probably came from a non-taken branch of an if
            observed = scope.get_var(name)
            check_compatible(name, expected, observed)
